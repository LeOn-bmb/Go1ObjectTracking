...
*/
        ...
        // Stereodaten
        size_t stereo_data_size = stereo.total() * stereo.elemSize();
        // Depthdaten
//        size_t depth_data_size = depth.total() * depth.elemSize();

        // Nachricht bauen
        Header header = {
            static_cast<uint32_t>(stereo_data_size),
//            static_cast<uint32_t>(depth_data_size),
            static_cast<uint32_t>(stereo.cols),
            static_cast<uint32_t>(stereo.rows),
//            static_cast<uint32_t>(depth.cols),
//            static_cast<uint32_t>(depth.rows),
            static_cast<uint32_t>(stereo.type()),
//            static_cast<uint32_t>(depth.type())
        };

        // Nachricht besteht aus: HEADER | Stereo-DATEN     | DEPTH-DATEN
//        size_t total_size = sizeof(header) + stereo_data_size + depth_data_size;
        size_t total_size = sizeof(header) + stereo_data_size;
        ...
        ptr += sizeof(header);
        // Stereo kopieren
        memcpy(ptr, stereo.data, stereo_data_size);
